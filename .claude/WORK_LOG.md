...
### 다음에 할 일 (TODO)
- [x] 다음 세션: 열려있는 `Player.prefab` 프리팹 스테이지 정리(있다면 닫기) 확인 → (계속 14)에서
  `close_prefab_stage`가 성공 처리됨(Sword.prefab 작업 중) - Player.prefab도 다음 세션 확인
...
### 사용자 플레이 테스트 피드백 4가지
"몬스터랑 일정거리 가까워지면 멈추긴 하는데 플레이어가 공격을 안하는 경우가 있는데? 그리고
검을 날 부분으로 휘둘러야지 이건 몽둥이 휘두르는거랑 차이가 없는데. 몬스터가 죽었다 다시
태어나면 빨간색으로 깜빡이는 경우가 잦아. 그리고 최적화 작업을 진행해줘."

**① 몬스터는 멈췄는데 공격 안 함 - 원인 특정**: `Monster`는 플레이어와의 거리를 수평(Y축
무시)으로 재는데, `PlayerCombat.FindNearestInRange()`는 Y축 포함 3D 거리로 재고 있었음 -
슬라임 스폰 위치가 플레이어 피벗보다 약간 높아서 경계선 근처에서 두 판정이 어긋났던 것.
`PlayerCombat`도 수평 거리로 통일해서 수정.

**② 검이 몽둥이 같음 - 기하학적 원인 특정**: (계속 12)에서 만든 3패턴 중 "수직 내려찍기"가
검의 로컬 X축(날의 폭 방향, 두 엣지가 있는 축)을 회전축으로 썼음 - 이 축으로 돌리면 날이
아니라 넓은 평평한 면이 진행 방향을 향한 채로 움직임(정확히 "몽둥이"처럼 보이는 원인). 검이
날 쪽(두께 축, 양쪽 평면과 나란한 로컬 Z축)으로 회전해야 얇은 날이 진행 방향을 리드함 - 3패턴
전부 **항상 로컬 Z축으로만 회전**하도록 재작성, "수직/수평/사선"으로 보이는 차이는 회전축이
아니라 검을 쥔 기본 방향(holdOrientation)만 바꿔서 만듦(회전축은 절대 안 바뀌니 날이 항상
리드함). 추가로 `Sword.prefab`의 날 폭도 0.06→0.11로 넓히고 길이는 0.95→0.85로 줄여서 얇은
날/넓은 평면의 시각적 대비 자체를 키움.

**③ 리스폰 시 빨간 깜빡임 - 원인 특정**: 처치타격이 `flashTimer`를 켜놓고(사망 판정보다 먼저
실행) 죽이는데, 죽으면 오브젝트가 비활성화되어 `Update()`가 안 돌면서 그 타이머가 그대로
얼어붙어 있었음 - 리스폰으로 재활성화되는 순간 얼어붙은 타이머를 보고 또 빨갛게 깜빡였던 것.
`Monster.ResetAt()`에서 `flashTimer`/색을 명시적으로 초기화해서 수정.

**④ 최적화**: `PlayerCombat`이 매 공격 판정마다 쓰던 `FindObjectsByType<Monster>()`(씬 전체
스캔)를 `FieldMonsterSpawner`가 이미 들고 있는 몬스터 목록(`IReadOnlyList<Monster> Monsters`
신규 노출) 재사용으로 교체. `Monster`도 `Renderer.material`을 매 프레임 다시 읽던 걸 최초
1회만 읽어 캐싱(`Material bodyMaterial` 필드)하도록 정리.

`Sword.prefab`의 날 크기 조정은 `open_prefab_stage`(이번엔 저장+닫기 둘 다 성공)로 처리.
`combat_design_v1.html`에 §4 재수정 + §6(버그 수정/최적화) 신규 추가(v1.4), 브라우저로 다시
열어둠.

컴파일: `read_console` 확인(여러 회) - 에러 0건.

### 다음에 할 일 (TODO)
- [x] 다음 세션: Play Mode에서 검이 실제로 날 쪽으로 휘둘러지는지, 근접 공격 누락 버그 해소,
  리스폰 시 빨간 깜빡임 해소를 눈으로 직접 확인 → 사용자가 직접 플레이해서 추가 피드백 줌,
  (계속 15)로 이어감
- [ ] ② 재료·무기 다양화 착수
...
### 사용자 플레이 테스트 피드백 3가지 더
"캐릭터가 장시간 멀리 있으면 판매 테이블로 순간이동하는건 없애주고, 현재 검을 휘두를 때
몬스터를 바라보면 좋겠어. 그리고 휘두는 검 모션이 많이 어색한 것 같아. 첫번째 휘두를 땐
괜찮은데 두번째 세번째 휘두르는건 플레이어 몸을 관통하고 심지어 몬스터를 향해서 휘두르는
느낌도 없어."

**① 사망 시 판매대 순간이동 제거**: 사용자가 "장시간 멀리 있으면"이라고 표현했지만 실제
메커니즘은 시간/거리 트리거가 아니라 **몬스터 접촉 피해로 HP가 0이 되는 사망-리스폰**이었음
((계속 9)에서 만든 기능) - 사냥터가 채집지보다 더 뒤로 밀려난 뒤(계속 13) 죽을 때마다 먼
판매대까지 걸어와야 해서 더 거슬리는 경험이 된 것으로 추정. `PlayerHealth.Die()`에서
`SalesCounter`로 순간이동시키던 부분 제거 - 이제 그 자리에서 HP만 회복 + 무적시간만 부여.

**② 몬스터 조준**: `PlayerCombat`이 지금까지 플레이어 방향을 전혀 신경 안 썼음 -
`PlayerWoodcutting`이 나무를 바라보는 것과 동일한 패턴(`FaceTarget` + `!HasMovementInput` 게이트)을
추가, 사정거리 안에 몬스터가 있으면(공격 쿨다운 중이어도) 매 프레임 그쪽을 바라보게 함.

**③ 검이 몸을 관통 - 기하학적 원인 추가 특정**: (계속 14)에서 "3패턴 모두 로컬 Z축(날 리딩)
으로만 회전"하도록 고쳤지만, **회전축은 맞았어도 "쥔 방향"(holdOrientation) 자체가 틀렸던
패턴이 있었음** - "수평" 패턴이 `Euler(-90,0,0)`을 써서 검 길이축이 로컬 **-Z(몸 뒤쪽)**를
향한 채 시작했음(첫 스윙이 우연히 "수직"이면 괜찮고 이후 "수평"/"사선"이 걸리면 관통해
보였던 것 - "첫번째는 괜찮은데 두세번째는 이상하다"는 사용자 관찰과 정확히 일치). 부호를
`Euler(90,0,0)`으로 바꿔 길이축이 로컬 **+Z(정면)**를 향하게 수정, "사선" 패턴도 손으로 고른
각도 대신 수직·수평 두 방향의 `Quaternion.Slerp` 중간값으로 바꿔 같은 실수 재발 방지. ②(몬스터
조준)와 합쳐지면 이제 "몸 밖으로, 몬스터를 향해" 휘둘러짐.

`combat_design_v1.html`에 각각 반영(v1.5), 브라우저로 다시 열어둠.

컴파일: `refresh_unity(compile=request)` + `read_console` 확인 - 에러 0건.

### 다음에 할 일 (TODO)
- [x] 다음 세션: Play Mode에서 사망 시 순간이동 안 하는지, 전투 중 몬스터를 바라보는지, 검이
  몸을 관통하지 않고 몬스터 쪽으로 휘둘러지는지 확인 → 사용자가 직접 플레이, (계속 16)으로 이어감
- [ ] ② 재료·무기 다양화 착수
...
### 사용자 피드백 - 사망 연출 재도입 + 타격 이펙트 + 추가 아이디어 요청
"사망 처리였구나. 차라리 페이드 인아웃이 되면서 자연스럽게 순간이동 되는 연출이 더 좋을 것
같아. 캐릭터가 쓰러지고 난 후에 그리고 몬스터를 때렸을 때 타격 이펙트나 카메라 연출 등 있었으면
좋겠어. 현재 너무 허전해. 뭔가 더 추가할만한 아이디어가 있다면 제시해줘."

**해석**: (계속 15)에서 뺐던 순간이동을 완전히 없애자는 게 아니라, "예고 없이 화면이 뚝 끊기는"
연출 부재가 문제였던 것으로 재정리 - 순간이동은 되살리되 쓰러짐→페이드아웃→(화면 검은 동안
순간이동)→페이드인→기립으로 감쌈.

### 구현
- **`HitEffects.cs`(신규, `Combat/`)**: `OreNode`/`WoodNode`가 이미 쓰던 "조각 N개가 튀어나가며
  축소·회전 후 소멸" 기법을 그대로 재사용(새 기법 발명 안 함) - 다만 몬스터가 전부 런타임
  생성이라 프리팹 에셋 없이 코드로 스파크 템플릿 생성(`CarryItemTemplates`와 동일 요령). 플레이어
  타격=골드 스파크, 피격=붉은 스파크, 처치=더 큰 초록 버스트로 구분. 크래프팅 미니게임에서 이미
  쓰던 `CameraFollow.Shake()`도 그대로 재사용(새 카메라 시스템 안 만듦) - 피격 쪽을 타격보다
  살짝 더 크게.
- **`ScreenFade.cs`(신규, `UI/`)**: 전체화면 검은 오버레이, `FadeOutAndIn(duringBlack, onComplete)`
  로 "페이드아웃→콜백 실행(화면 까만 동안)→페이드인→완료 콜백" 순서 보장.
- **`PlayerDeathPresentation.cs`(신규, `Player/`)**: `PlayerHealth.OnDeath` 구독, 캐릭터 모델
  전방 쓰러짐(0.35초) → `ScreenFade`로 감싼 대장간 순간이동 → 모델 기립까지 전체 시퀀스 담당.
  `PlayerHealth`는 여전히 순수 정적 데이터 클래스로 유지(좌표 이동/코루틴 로직은 전부 이쪽으로).
- **`PlayerHealth.TakeDamage`가 `bool` 리턴하도록 변경**(실제로 데미지가 들어갔는지) -
  `Monster`가 무적 시간 중 헛스윙까지 타격 이펙트를 재생하지 않도록.
- `ResourceHUD.Start()`에 `PlayerDeathPresentation.Instance.Activate()` 추가.

### 추가 아이디어 제안 (미착수, `combat_design_v1.html` §9에 기록)
데미지 숫자 팝업(우선순위 제안) / 저체력 화면 비네트(`ScreenFade`와 같은 오버레이 기법 재사용
가능) / 몬스터 처치 텍스트 팝업 / 효과음(게임플레이용 오디오 시스템 자체가 없어서 범위 밖 보류).

`combat_design_v1.html` §4/§8/§9 갱신(v1.6), 브라우저로 다시 열어둠.

컴파일: 새 파일 3개의 `.meta` 생성 확인 후(=실제 임포트 반영) `read_console` 에러 0건.

### 다음에 할 일 (TODO)
- [x] 다음 세션: Play Mode에서 타격 스파크 색 구분(골드/빨강/초록), 카메라 흔들림, 사망 시
  쓰러짐→페이드→순간이동→페이드인 시퀀스 전체를 눈으로 확인 → 사용자가 직접 플레이, 추가
  피드백 줌 (계속 17)
- [ ] ② 재료·무기 다양화 착수
...
### 사운드는 최종 마무리 때 - 메모리 기록
"사운드는 최종적으로 마무리할 때, 넣을 예정이야" - (계속 16) §9에 "효과음(범위 밖 보류)"로
적어뒀던 항목의 시점을 명확히 확정. 세션이 끝나도 남아야 하는 프로젝트 결정이라
`project_sound_deferred_to_final_polish` 메모리로 별도 기록(현재 게임플레이 코드엔 오디오
시스템 자체가 없다는 사실도 같이 적어둠 - 나중에 다시 확인할 필요 없게).

### 사용자 피드백 - 플레이어도 맞았을 때 효과가 있었으면
"플레이어도 맞았을 때, 효과가 있었으면 좋겠어" - (계속 16)의 `HitEffects.MonsterHitPlayer()`는
스파크/카메라 흔들림 둘 다 플레이어 "근처"에서만 일어나고 정작 몸에는 반응이 없었던 공백.
몬스터가 맞을 때 이미 쓰던 빨간 플래시(`bodyMaterial.color` 잠깐 붉게, `Monster.cs` 패턴)를
플레이어 모델(`Player/Model`)에도 동일 적용 - `HitEffects`에 `playerMaterial`/`playerBaseColor`
캐싱 + `PlayerFlashRoutine()` 추가. `PlayerDeathPresentation`이 이미 쓰던 것과 같은 방식
(`transform.Find("Model")`)으로 안전하게 참조(`GetComponentInChildren<Renderer>()`는 등짐
아이템 렌더러가 먼저 걸릴 수 있어 피함).

`combat_design_v1.html` §8에 반영(v1.7), 브라우저로 다시 열어둠.

컴파일: `read_console` 확인 - 에러 0건.
...
### 사용자 리포트 - 망치질 중 터치하면 조이스틱이 켜져서 캐릭터가 움직임
"현재 망치질 부분에서 터치를 하면 조이스틱이 활성화하고 캐릭터가 움직여." + 이어서 유사 버그/
미최적화 코드 전반을 검토해달라는 요청.

**원인 특정**: `CraftingMinigameUI`(온도조절 풀무질 + 해머링 미니게임)의 캔버스가
`sortingOrder`를 아예 설정 안 해서 기본값 0 - `FloatingJoystick`의 전체화면 터치 존이 있는
"HUD Canvas"도 똑같이 0. 두 캔버스가 같은 순번이라 레이캐스트 우선순위가 불안정해서, 미니게임
화면을 터치해도 조이스틱이 같이(또는 대신) 반응해서 캐릭터가 움직였던 것. 이 프로젝트에서
이미 알려진 클래스의 문제 패턴(과거 "조이스틱 vs 판매 UI" 케이스, 그때는 판매 UI가 order 5라
문제 없었음 - 이번엔 미니게임 UI가 order를 아예 안 넣어서 재발). `CraftingSilhouetteUI`(같은
제작 흐름의 앞 단계)와 동일한 10으로 설정해서 해결 - 둘은 순차 실행이라 절대 동시에 안 뜸.

### 전체 캔버스 sortingOrder 감사
이 프로젝트의 모든 `ScreenSpaceOverlay` + `GraphicRaycaster` 캔버스를 전수 검사 - 조이스틱(0)과
같은 순번으로 충돌할 수 있는 건 `CraftingMinigameCanvas` 하나뿐이었음(나머지는 이미 명시적
순번이 있거나, `GraphicRaycaster`가 아예 없어서 클릭을 가로챌 수 없거나, WorldSpace라 무관).

### 검토 중 발견한 추가 버그/보완 사항 (전부 수정)
- **(계속 18) 회귀 버그**: `CraftingSilhouetteUI`의 재료 시트가 광물 보유량을
  `ResourceBank.Get(ResourceType.Ore)`로 표시하고 있었는데, 이 값은 이제 제작이 소모하지 않는
  "누적 예치량"으로 바뀌어서 실제로는 절대 안 줄어드는 잘못된 "보유 개수"가 표시되고 있었음 -
  `OreBank.TotalCurrent`로 교체.
- **죽은 코드**: `CraftingStation.oreType` 필드가 (계속 18)의 `OreBank` 전환 이후 아무 데서도
  안 쓰이는데 남아있었음 - 제거.
- **방어 코드**: `PlayerDeathPresentation`이 사망 연출 도중 또 사망 이벤트가 들어오면(이론상
  무적시간이 막아주지만) 두 코루틴이 동시에 캐릭터 모델 회전을 다투는 레이스가 가능했음 -
  `sequenceRunning` 플래그로 방어.

컴파일: `refresh_unity` 재요청 후 `read_console` 확인 - 에러 0건.

### 다음에 할 일 (TODO)
- [ ] 다음 세션: Play Mode에서 망치질 중 터치해도 캐릭터가 안 움직이는지, 재료 시트의 광물
  보유량 표시가 정확한지 확인
- [ ] (계속 18)의 TODO(광물 채굴→제작 한 바퀴 검증)와 함께 확인
...
### 사망 시 등짐 소실
"그리고 죽게 되면, 플레이어 등에 있는 자원들은 다 사라지게 해줬으면 좋겠어." - 지금까지
"골드/재료 손실 없음"이던 사망 방침을 일부 수정 - 이미 보관함에 예치한 자원/골드/제작한 도구는
안전하게 유지하고, **아직 등에 짊어진(미예치) 목재/광물/마석만** 잃도록 함.

- `CarryStack.ClearAll()`(신규) - `LayerCount`로 전체 레이어를 순회하며 기존 `Clear(layer)`
  재사용(레이어별 하드코딩 없음, 나중에 레이어가 늘어도 그대로 작동).
- `PlayerDeathPresentation`의 화면이 검은 동안 실행되는 콜백을 `TeleportToSmithy` 단독에서
  `HandleBlackout`(순간이동 + 등짐 소실을 함께 수행)으로 교체 - 죽으면 즉시가 아니라 페이드
  중간(안 보이는 동안)에 사라지도록 해서 기존 사망 연출과 자연스럽게 맞물림.
- `combat_design_v1.html`/`PlayerHealth.cs` 클래스 주석에서 "골드/재료 손실 없음" 표현을
  "등짐만 위험해진다"로 정정(v1.8).

컴파일: `refresh_unity` 재요청(첫 확인은 새 메서드 반영 전 스냅샷이라 무효, 재확인 후) -
에러 0건.

### 다음에 할 일 (TODO)
- [x] 다음 세션: Play Mode에서 사망 시 등짐(목재/광물/마석)이 실제로 비워지는지, 보관함에
  이미 넣어둔 것과 골드는 그대로인지 확인 - 사용자가 직접 플레이(마석 드랍은 확률상 정상 확인),
  (계속 21)로 이어감
- [ ] (계속 19)/(계속 18) 남은 TODO(망치질 터치, 광물 표시 등)도 함께 확인

---

## 2026-08-12 (계속 21)

### 로드맵 ② 다음 갈래 - 무기 종류 다양화 vs 마석 인챈트 중 선택
"다음 작업을 이어서 진행해보자" → 도끼/망치/단검(무기 종류 확장)은 진짜 "장착" 개념이 필요해져서
(등급처럼 자동 최고값 대체가 안 통함 - 검이랑 도끼는 서로 대체재가 아님) 사용자에게 두 갈래
제시: ①무기 종류+간단 장착 UI ②마석 인챈트(기존 구조 재사용, 장착 UI 불필요). 사용자가 ②를
선택하되 **"결국 다양한 도구/무기가 추가될 거니 검 전용으로 몰아넣지 말고 유동적으로 확장 가능한
구조로 만들어달라"**고 명시.

### 구현 - 마석 인챈트 + 처음부터 다축 확장 구조로
- **`WeaponType.cs`(신규)**: 지금은 `Sword` 하나뿐인 enum이지만, `CraftingStation`이 이 필드를
  갖도록 만들어서 나중에 도끼 대장간은 "enum 값 하나 + 새 스테이션 인스턴스"만 있으면 됨.
- **`ManaElement.cs`(신규)**: 화염/독/번개/냉기, `WeaponType`과 완전 독립. 상태이상 수치
  (화상/독 도트, 번개 마비, 냉기 슬로우)와 스파크 색상 매핑도 여기 보관.
- **`ToolInventory.cs`**: 키를 `(WeaponType, OreGrade, CraftGrade, ManaElement)` 4축 튜플로
  확장(`IEquatable` 구조체 키) - "검 전용" 필드/메서드 없이 전부 `WeaponType` 매개변수로 받음.
  `TryGetBestWeapon(out weapon, out oreGrade, out element)` 신규.
- **`CraftingStation.cs`**: `weaponType` 필드 추가. `manaSpent > 0`일 때 무작위 속성을 굴려서
  결과에 태그(필드산 마석은 아직 속성이 없어서 "저품질이라 예측 불가"로 설명) - Quick Craft는
  마석 슬롯이 없어서 인챈트 안 붙음.
- **`Monster.cs`**: `ApplyStatusEffect(ManaElement)` 신규 - 화상/독은 도트 틱(도트로 죽어도
  처치 이펙트는 재생하되 마석 드랍은 안 굴림 - 그건 직접 타격 전용으로 남김), 번개는 이동/공격
  정지, 냉기는 이동속도 50% 감소. 상태는 동시에 하나만(새로 걸리면 덮어씀). 히트 플래시가 없는
  동안엔 속성색과 기본색을 절반 섞어서 지속 상태를 표시.
- **`PlayerCombat.cs`**: `ToolInventory.BestOreGrade` 대신 `TryGetBestWeapon`으로 등급+속성을
  함께 읽어와서 공격력 계산 + (처치 아닐 때만) `Monster.ApplyStatusEffect` 호출.
- **`HitEffects.cs`**: `PlayerHitMonster`에 색상 오버로드 추가 - 인챈트 무기면 속성색 스파크.
- **`CraftingMinigameUI.ShowGradeResult`**: 인챈트 있으면 "Fire Steel Fine!"처럼 속성을 앞에
  붙이고 결과 텍스트 색도 속성색으로.

컴파일: 첫 확인 때 `Monster.ApplyStatusEffect` 없다는 에러 1건 - `Monster.cs` 저장이 아직
반영 전인 스냅샷이라 무효, `refresh_unity` 재요청 후 에러 0건 확인.

`weapon_diversity_design_v1.html` §6(v1.1) 신규 추가 + §1 범위 갱신, 브라우저로 다시 열어둠.

### 다음에 할 일 (TODO)
- [ ] 다음 세션: Play Mode에서 마석 넣고 제작 시 속성이 실제로 붙는지("Fire Steel Fine!" 등),
  화상/독/마비/냉기 효과가 몬스터에게 실제로 작동하는지, 스파크 색이 속성별로 다른지 확인
- [ ] 도끼/망치/단검 - 구조는 준비됐으니 실제 착수는 사용자 지시 대기

---

## 2026-08-12 (계속 22)

### 사용자 요청 3가지 - 하이어라키 정리, 터치 인벤토리, 재료선택 UI 개선
"현재 풀링되는 오브젝트나 아이템들은 풀링을 모아두는 오브젝트 자식으로 정리가 될 수 있게 해줬으면
해. 너무 하이어라키에 길어지게 있다보니 복잡해져. 그리고... 터치를 이용해서 하는 게임이라
인벤토리도 오른쪽 중단 쯤? 아이콘을 누르면 열리도록... 그리고 현재 재료선택 UI를 개선해보자."

### ① 하이어라키 정리
- **`GameObjectPool.cs`**: 자기 자신 아래 `Pooled Instances` 컨테이너 신설 - `Spawn()`의 신규
  인스턴스화/재사용 경로 둘 다, `Despawn()`도 전부 이 컨테이너로 (재)부모 설정. 캐리 중이던
  아이템이 등에 붙어있다가 반납되는 경우처럼 중간에 다른 부모로 옮겨졌던 것도 `Despawn()` 한
  곳에서 전부 정리됨.
- **`RuntimeSystemsRoot.cs`(신규)**: `GuidedTutorial`/`CraftingMinigameUI`/`PlayerCombat`/
  `HitEffects`/`ScreenFade` 등 자체 부팅 싱글턴 13개 + 템플릿 오브젝트 2개(`CarryItemTemplates`,
  `HitEffects`의 스파크 템플릿)가 전부 이 아래로 - 씬 최상단에 낱개로 흩어져 있던 것들을 `~Runtime
  Systems` 하나로 묶음. `DontDestroyOnLoad`는 반드시 루트 오브젝트에서만 되므로, 각 싱글턴이
  `DontDestroyOnLoad(go)` 호출 **후**에 이 루트 밑으로 재부모 설정하는 순서 유지.
  `GameObjectPool.Instance`도 동일하게 이 루트 아래로.
- **`Monster.Spawn`/`Customer.Spawn`**에 선택적 `parent` 매개변수 추가 - 각각
  `FieldMonsterSpawner`/`CustomerVisualManager` 자기 자신을 부모로 넘겨서 스폰된 슬라임/손님들이
  스포너 밑에 정리되도록.

### ② 터치 인벤토리
- **`ToolInventory.AllOwned()`** 신규 - 보유 중인 모든 (무기·광물등급·품질·마석속성) 조합을
  개수와 함께 열거(등급 내림차순).
- **`PlayerInventoryUI.cs`**(신규, `UI/`) - "BAG" 아이콘(우측 가장자리, 퀘스트 카드와 HP 바
  사이 빈 구간 -256, 실측 좌표 기반) 탭하면 목록 패널 토글. 인챈트 있는 항목은 행이 속성색으로
  옅게 물듦.

### ③ 재료선택 UI 개선
바로 진행하기엔 "어떤 부분을 개선하고 싶은지"가 좀 더 필요해서(터치 제스처? 정보 표시? 등급
반영?) 사용자에게 구체적 방향 확인 중 - 다음 응답에서 이어감.

컴파일: `refresh_unity` 재요청 후 `read_console` - 에러 0건.

`weapon_diversity_design_v1.html` §7(v1.2) 신규 추가, 브라우저로 다시 열어둠.

### 다음에 할 일 (TODO)
- [ ] 다음 세션: Play Mode에서 하이어라키가 실제로 `~Runtime Systems` 밑으로 정리됐는지,
  인벤토리 아이콘/패널이 정상 동작하는지 확인
- [ ] 재료선택 UI 개선 - 사용자 방향 확인 후 착수

## 2026-08-14 (계속 23)

### 재료선택 UI 개선 - 사용자 방향 확인 회신
"1) 안 쓰는 html 문서 정리 2) 가끔 크래프트 버튼 눌러도 취소되는 버그(버튼 간격 문제로 추정)
3) 마석 등급(최하급~최상급) 시스템, 필드는 일단 최하급만 4) 재료선택 UI 레이아웃/가독성 개선.
3)/4) 착수 전 브리핑 문서로 보완 아이디어 제시 후 진행."

### ① 안 쓰는 HTML 문서 정리 - 완료
코드/문서 상호 참조 grep으로 교차 확인 후 `git rm`:
- `customer_order_design_v2.html`, `customer_order_design_v3.html` - v4가 전면 대체, 현재 코드는
  v4만 참조(v1은 §1/§2/§5/§6이 아직 코드에서 직접 인용되어 유지)
- `tutorial_design.html` - guided_tutorial_design.html이 명시적으로 대체
- `tutorial_guide.html` - 전투/마석 도입 이전 초안(당시 "몬스터 없음" 서술), 어디서도 미참조

### ② FORGE/CANCEL 버튼 겹침 버그 - 완료
`CraftingSilhouetteUI.cs` - 두 버튼 290px폭·중앙 간격 30px로 붙어있던 것을 260px폭·간격 140px로
확대(중심 ±160→±210, 높이 100→110). 컴파일 확인(에러 0건).

### ③④ 마석 등급 시스템 + 재료선택 UI 레이아웃 - 브리핑 작성, 구현 대기
`mana_grade_and_ui_design_v1.html`(신규) 작성 후 브라우저로 열어둠. 핵심 제안:
- `ManaGrade`(신규 enum, 5단계) + `ManaBank`(신규, `OreBank` 패턴 그대로 복제 - 등급별 재고 +
  누적 획득량 기반 `Ceiling`) - 필드 상한은 처음엔 최하급 고정, 문턱값만 올리면 하급까지 확장.
- `ToolInventory`의 Key에 `ManaGrade` 축 추가(4축→5축), `Monster.ApplyStatusEffect`에 배율
  파라미터 추가해 등급별 효과 배율(최하급 ×1.0 = 현재 수치 그대로 유지) 반영.
- 재료선택 시트는 광물과 동일하게 "자동 최고 등급 소모"로 제안(수동 선택 UI는 나중에).
- UI 레이아웃: 광물·목재(필수)/마석×3(선택) 그룹 분리 + 구분선, 슬롯 라벨 확대, FORGE 위
  결과 미리보기 줄 추가, 시트 칩 글자 확대.
사용자 확인 대기 중 - 승인 후 §1(마석 등급) → §2(레이아웃) 순으로 구현.
...
### 망치질 스파크 이펙트 + 발견한 경고 정리
"망치질할 때 스파크 튀거나 하는 이펙트가 추가되었으면 해. 그리고 다음 작업을 진행해보자.
개선이 필요한 것이 있다면 그것부터 잡고가자."

- **`CraftingMinigameUI.cs`** - `SpawnHammerSparks`/`HammerSparkRoutine` 신규: `HitEffects`의
  월드 스페이스 스파크 기법(방사형 튐+축소+페이드)을 UI RectTransform 버전으로 이식. 망치가
  블레이드에 닿는 순간(`PlayHammerSwing`의 downDuration 종료 시점)에 발동, Perfect(골드x10)/
  Good(주황x6)/Miss(무채색x3)로 색·개수 차등.
- **개선 먼저 처리**: 이전 턴에서 마석을 `ResourceBank` 대신 `ManaBank`로 옮기면서
  `CraftingStation.manaStoneType` 필드가 미사용 상태로 남아 컴파일 경고(CS0414) 발생 -
  필드 제거로 정리.

컴파일: `refresh_unity` 재요청 후 `read_console` - 에러/경고 0건.

### 다음 작업 - 세션 분리 제안
로드맵 순서(전투→②재료·무기 다양화→③해금 게이트→④암시장)상 다음 단계는 ②의 남은 부분
(도끼/망치/단검 실제 무기 종류 확장 + 장착 선택 UI - 지금은 "최고 등급 자동 장착"으로
땜빵 중)이나, 이건 새 UI 패러다임(장착 화면)이 필요한 제법 큰 기능이라 이번 세션(이미 매우
길어짐)보다 새 세션에서 시작하길 사용자에게 제안함.

## 2026-08-18 - UI 피드백 반영 (버튼 폭/무기 선택/장비창)

### 반영 내용
- `CraftingSilhouetteUI.cs` - FORGE/CANCEL 버튼 260→210 폭 축소. 안 보이던 `<`/`>` 무기 선택
  화살표를 SWORD/AXE/HAMMER/DAGGER 4버튼(항상 표시, 선택 시 앰버 강조)으로 교체 - 아래 콘텐츠
  블록 전체를 120px 하향 이동해서 자리 확보.
- `PlayerInventoryUI.cs` - "Inventory" → "Equipment"로 리브랜드. 상단에 EQUIPPED 카드(현재
  장착 무기 이름+ATK/SPD 배율) 신설. 행마다 DEL 버튼 추가 - 누르면 바로 삭제 대신 확인 다이얼로그
  ("Delete X x N? This cannot be undone.")를 거쳐야 실제 삭제(`ToolInventory.RemoveStack` 신규).

### 검증 - 스크린샷 대신 리플렉션 기능 검증
이 환경의 `manage_camera` screenshot이 Screen Space Overlay 캔버스를 못 잡아서(HUD/조이스틱도
안 보임 - 툴 한계, 내 코드 문제 아님) 눈으로 확인은 못 했음. 대신 `execute_code` 리플렉션으로
기능 검증: PlayerInventoryUI 장착/DEL 확인창/삭제 전부 정상 동작, CraftingSilhouetteUI
FORGE/CANCEL 실측 size=(210,100), 무기 버튼 4개 위치/탭 반응 정상.

**사고 처리**: 이 검증 도중 플레이모드가 아니라 에디트 모드에서 리플렉션 호출을 실행해버려서,
`PlayerInventoryUI`/`CraftingSilhouetteUI` 싱글턴이 씬에 그대로 남는 사고 발생 - 사용자가
"필요 UI가 다 없어졌다"고 오인할 만큼 화면을 어지럽힘. 두 스트레이 오브젝트 삭제로 정리 완료
(씬은 저장 안 해서 디스크엔 영향 없었음). 실제 게임 UI(배속/오토/퀘스트 등)는 전부 런타임에만
생성되는 구조라 애초에 에디트 모드에선 안 보이는 게 정상 - 삭제와 무관.

### 사용자 피드백 반영
- "3일 안에 가능하냐"는 질문은 별도 스프린트 계획이 아니라 기존 작업 계속 진행하라는 뜻이었음 -
  만들었던 `polish_sprint_plan_v1.html`은 스코프 오해였어서 제거.
- 앞으로 작업 단위가 끝나면 항상 커밋 습관화 (사용자 명시적 재확인).
...
## 2026-08-21 - 손님 줄서기 다듬기 + 안 쓰는 HTML 문서 정리

### 손님 요구치 표시 시점 / 줄 중앙정렬 / 간격
- 말풍선(요구치)이 스폰 직후부터 보이던 것을, 맨 앞자리(queue[0])에 실제로 도착(`HasArrived`)한
  뒤에만 뜨도록 게이팅(`Customer.SetBubbleVisible`).
- `QueuePoint0~4`를 카운터 폭 밖으로 옆으로 늘어서던 배치 → 카운터 정면 중앙(x=0) 일렬로 재배치.
- 캡슐끼리 붙어 보인다는 피드백으로 슬롯 간격 0.9m → 1.6m 확대(SpawnPoint도 동일 간격 유지).
- 커밋 3건(요구치 시점+중앙정렬, 간격 확대) 완료, 컴파일 에러/경고 0건.

### 안 쓰는 HTML 문서 정리
grep 상호참조로 교차 확인 후 `git rm` 4개(mana_grade_and_ui_design_v1.html §0의 이전 정리와 동일
절차):
- `customer_order_design_v4.html`/`v5.html`/`v6.html` - v7이 전면 대체, 현재 코드는 v7만 참조
  (당시엔 v4가 참조 중이었지만 이후 v7로 넘어가면서 끊김)
- `dungeon_design_v2.html` - v1 대체를 표방했지만 실제로 채택되지 않음(현재 던전 코드는 전부
  `dungeon_design_v1.html`만 참조, v2는 어디서도 참조되지 않는 미채택 제안)
유지: `customer_order_design.html`(§1/§2/§5/§6 코드 직접 참조), `guided_tutorial_design.html`
(v2가 구조상 대체했지만 다른 문서들이 §9/§10 앵커로 여전히 참조), 나머지 전부 현재 코드 참조 중.

### 기획 문서 목차 신설
"기획서를 하나의 큰 파일로 묶어두고 거기서 컨텐츠별로 나뉘었으면" 요청 - 15개 문서를 전부 한
파일로 합치는 대신(편집할 때마다 무관한 시스템까지 뒤져야 함), `design_doc_index.html`(신규) 하나가
